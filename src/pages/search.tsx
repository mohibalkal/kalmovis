    ...
    <div className={styles.MoviePage}>
      ...
      <div className={styles.InputWrapper}>
        <input
          ...
          placeholder={t("search.enterThreeChars")}
      ...
      <div className={styles.movieList}>
        ...
        {query.length > 2 && data?.length === 0 ? (
          <h1>{t("common.noResults")}</h1>
        ) : null}
        {query.length > 2 && data === undefined
          ? dummyList.map((ele, index) => (
              <Skeleton key={`skeleton-${index}`} className={styles.loading} />
            ))
          : null}
        {genreListMovie?.length === 0 || genreListTv?.length === 0
          ? dummyList.map((ele: any, index) => {
              return (
                <MovieCardLarge
                  key={`dummy-${index}`}
                  data={ele}
